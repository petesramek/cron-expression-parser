...
pub fn next_occurrence(parsed: &Cron, reference_timestamp: i64) -> Result<i64, CronError> {
    let mut current = DateTime::<Utc>::from_timestamp(reference_timestamp, 0)
        .ok_or(CronError::InvalidTimestamp(reference_timestamp))?;
    ...
    Err(CronError::NoOccurrenceFound)
}
...
mod tests {
    ...
    fn finds_next_minute_for_all_wildcards() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 1, 12, 0, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();
        let expected = Utc.with_ymd_and_hms(2026, 1, 1, 12, 1, 0).unwrap().timestamp();
    ...
    fn finds_next_midnight() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 1, 12, 30, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();
        let expected = Utc.with_ymd_and_hms(2026, 1, 2, 0, 0, 0).unwrap().timestamp();
    ...
    fn finds_next_specific_weekday_time() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 6, 9, 0, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();

        // Next Monday is January 12, 2026 at 08:30 UTC
        let expected = Utc.with_ymd_and_hms(2026, 1, 12, 8, 30, 0).unwrap().timestamp();
    ...
    fn handles_step_expression() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 1, 12, 7, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();
        let expected = Utc.with_ymd_and_hms(2026, 1, 1, 12, 15, 0).unwrap().timestamp();
    ...
    fn handles_range_step_expression() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 1, 9, 30, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();
        let expected = Utc.with_ymd_and_hms(2026, 1, 1, 11, 0, 0).unwrap().timestamp();
    ...
    fn handles_month_boundary_rollover() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 31, 23, 59, 0).unwrap().timestamp();
    ...
    fn handles_day_of_month_overflow() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 4, 1, 0, 0, 0).unwrap().timestamp();

        let next = next_occurrence(&cron, reference).unwrap();

        // April has no 31st, so next should be May 31
        let expected = Utc.with_ymd_and_hms(2026, 5, 31, 0, 0, 0).unwrap().timestamp();
    ...
    fn handles_year_rollover() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 12, 31, 23, 59, 0).unwrap().timestamp();
    ...
    fn returns_no_occurrence_for_impossible_schedule() {
        ...
        let reference = Utc.with_ymd_and_hms(2026, 1, 1, 0, 0, 0).unwrap().timestamp();

        let result = next_occurrence(&cron, reference);

        assert!(matches!(result, Err(CronError::NoOccurrenceFound)));
    }
}